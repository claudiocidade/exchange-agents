...
namespace AutoTrader.Console.Exchanges.Cryptopia
{
    ...
    public class CryptopiaExchangeClient : ExchangeClient, ICryptopiaExchangeClient
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CryptopiaExchangeClient"/> class.
        /// </summary>
        /// <param name="client">An instance of the <see cref="IRestClient"/> 
        /// to be used to manipulate the exchange API.</param>
        public CryptopiaExchangeClient(IRestClient client) : base(client)
        {
        }
        ...
        public override async Task<long> CreateOrder(string symbol, double bidPrice, double amount, OrderSide side)
        {
            ...
            parameters.Add(new Parameter { Name = "price", Value = $"{bidPrice:F6}", Type = ParameterType.GetOrPost });
        ...
        public override async Task<OrderStatus> CheckOrderStatus(string symbol, long orderId)
        {
            ...
            Order result = JsonConvert.DeserializeObject<Order>((await this.GetRequestResult(request)).Content);
